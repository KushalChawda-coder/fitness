...
<div class="container">
  <div class="row mb-3">
    <div class="col-12 col-xl-12">
     <h1 class="text-center fw-bold">{{ $data->brand_name ?? 'Get Your Free Ebook!' }} </h1>
    </div>
  </div>
  <div class="row">
    <div class="col-12 col-xxl-6 offset-xxl-3">
      <div class="row">
        <div class="col-12 col-xl-7">
          <img src="{{ $data->banner_image ?? '' }}" alt="captcha" class="img-fluid">
        </div>
        @if(!isset($_GET['lead_id']))
	        <div class="col-12 col-xl-4">
	        	<form action="{{ route('generate_lead') }}" method="post" enctype="multipart/form-data">
	          		@csrf
	          	  <input type="hidden" name="digital_files" value="{{ $data->digital_files ?? '' }}">
	          	  <input type="hidden" name="status" value="{{ \App\Models\admin\LeadsActivity::REQUEST_E_BOOK }}">
	          	  @if(in_array('name', $column_name))
		          <div class="form-group mb-3">
		            <label>Enter Your First Name</label>
		            <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="name" class="form-control" required>
		            @error('name')
		          ...
		          <div class="form-group mb-3">
		             <button type="submit" class="btn btn-success w-100">{{ $data->action_button_name ?? 'Give me the free resource' }}</button>
		          </div>
		        </form>
	        </div>
        @else
